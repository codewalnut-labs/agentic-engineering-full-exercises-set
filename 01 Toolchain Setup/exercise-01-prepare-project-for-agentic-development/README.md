...
Your team is about to start working on a project with coding agents. Each developer relies on personal prompts and locally installed skills. Agents start implementing unclear requests, guess repository conventions, and review changes without checking the original requirements.

Your mission is to prepare a shared project setup: a useful `AGENTS.md` and agent skills for clarifying requirements, research, test-driven development, and code review. Agent skills are reusable instructions that guide a coding agent through a specific task. Your team should be able to use this setup to follow a consistent engineering workflow.
...
[agent-setup-app](./agent-setup-app) is a React and TypeScript support case-routing application with basic notes and a working baseline. The [readiness scenarios](./docs/readiness-scenarios.md) include an unclear upcoming request, a technical research question, a testing task, and a supplied change to review.
...
2. Create `agent-setup-app/AGENTS.md` and install and configure skills for those four tasks. Document the setup so another developer can use the same skills. Resolve their dependencies and align their instructions with the project's conventions.
...
Follow the [setup and verification instructions](./docs/setup.md) and [submission standard](../../docs/SUBMISSION_STANDARD.md). Run `npm run verify:exercise` from `agent-setup-app/` before raising a focused PR from your fork.
