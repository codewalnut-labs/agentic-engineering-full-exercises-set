...
Use the repository's supported Node.js range (22.12 to 24), npm, Git, a coding agent, and internet access for skill acquisition and primary-source research. Run `npm ci` and `npm run agent:check` from `agent-setup-app/`. Browser automation and external issue-tracker accounts are not required.
...
All commands below run from `agent-setup-app/`.
