...
test("workspace policy can remove an owner's invitation permission", () => {
  const state = makeState([makeInvitation()]);
  state.policy.inviteRoles = ["admin"];
  const original = structuredClone(state);
  expectRejected(createInvitation(state, createInput()), "UNAUTHORIZED", original);
  expectRejected(revokeInvitation(state, { invitationId: "INV-401", actorId: "USR-201", now: NOW }), "UNAUTHORIZED", original);
  assert.deepEqual(state, original);
});

test("expiry uses the policy value rather than a hard-coded week", () => {
  const state = makeState();
  state.policy.defaultInviteExpiryDays = 2;
  const result = createInvitation(state, createInput());
  assert.equal(result.invitation?.expiresAt, "2026-08-11T10:00:00.000Z");
});

test("an invitation expiring exactly now permits a new invitation with a new ID", () => {
  const state = makeState([makeInvitation({ expiresAt: NOW })]);
  const original = structuredClone(state);
  const result = createInvitation(state, createInput());
  assert.equal(result.ok, true);
...
test("accepting a guest normalizes member identity and cannot be repeated", () => {
  const state = makeState([makeInvitation({ role: "guest", email: "  Guest@Example.Test  " })]);
  const accepted = acceptInvitation(state, { invitationId: "INV-401", memberId: "USR-501", now: NOW });
  assert.equal(accepted.ok, true);
  assert.equal(accepted.state.members.at(-1)?.email, "guest@example.test");
  assert.equal(accepted.state.members.at(-1)?.role, "guest");
  const original = structuredClone(accepted.state);
  expectRejected(acceptInvitation(accepted.state, { invitationId: "INV-401", memberId: "USR-502", now: NOW }), "INVITATION_FINAL", original);
  assert.deepEqual(accepted.state, original);
});

test("finalized invitations allow a new invitation but never a reused ID", () => {
  for (const status of ["accepted", "revoked"] as const) {
    const state = makeState([makeInvitation({ status })]);
    assert.equal(createInvitation(state, createInput()).ok, true);
    expectRejected(createInvitation(state, createInput({ invitationId: "INV-401" })), "DUPLICATE_INVITATION_ID", structuredClone(state));
  }
});

test("rejected creation preserves deeply frozen input and result state", () => {
  const state = makeState();
  const original = structuredClone(state);
  ...
  freeze(state);
  expectRejected(createInvitation(state, createInput({ email: "IRIS@EXAMPLE.TEST" })), "MEMBER_EXISTS", original);
  assert.deepEqual(state, original);
});
