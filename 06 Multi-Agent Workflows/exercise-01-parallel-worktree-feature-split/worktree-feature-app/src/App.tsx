...
export default function App() {
  ...
  const selected =
    items.find((item) => item.id === selectedId) ?? initialWorkItems[0]

  const metrics = [
    { label: "Visible work", value: filteredItems.length, hint: `${items.length} total items` },
    { label: "High priority", value: items.filter((item) => item.priority === "High").length, hint: "requires attention" },
    { label: "Blocked", value: items.filter((item) => item.status === "Blocked").length, hint: "integration risk" },
