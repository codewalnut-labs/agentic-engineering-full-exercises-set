export function evaluateRenewalEligibility(account) {
  ...
  if (account.tier === "enterprise" && account.monthsActive >= 12) {
  ...
  if (account.tier === "pro" && account.monthsActive >= 6 && account.latePayments === 0) {
    return { status: "eligible", discountPercent: 10, reason: "pro-tenure" };
  }

  return { status: "ineligible", discountPercent: 0, reason: "plan-not-supported" };
}
